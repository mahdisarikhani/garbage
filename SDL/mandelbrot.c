...
point_t to_complex(SDL_Point p, size_t width, size_t height) {
    ...
    point_t fp = {.x = (double)(p.x - xoffset) / (double)scale * 4.0 - 2.0,
...
SDL_Point to_pixel(point_t p, size_t width, size_t height) {
    ...
    size_t xoffset = (width - scale) / 2;
    size_t yoffset = (height - scale) / 2;
    SDL_Point fp = {.x = (p.x + 2.0) / 4.0 * scale + xoffset, .y = (p.y + 2.0) / 4.0 * scale + yoffset};
    return fp;
}
...
void mandelbrot_set(size_t *set, size_t width, size_t height, size_t max_iter) {
...
void draw_mandelbrot_path(SDL_Renderer *renderer, SDL_Point mouse_pos, size_t max_iter, size_t width, size_t height) {
